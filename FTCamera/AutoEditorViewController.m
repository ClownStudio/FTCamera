...
- (void)initEditBar{
    ...
    UIButton *filterbutton = [self getAddMoreButton];
    CGRect filtertemp = filterbutton.frame;
    filtertemp.origin.x = [_filterPlistArray count] * 85;
    filterbutton.frame = filtertemp;
    [_filterScrollView addSubview:filterbutton];
    [_filterScrollView setContentSize:CGSizeMake(filterbutton.frame.origin.x + filterbutton.frame.size.width, 0)];
    ...
    UIButton *texturebutton = [self getAddMoreButton];
    CGRect texturetemp = texturebutton.frame;
    texturetemp.origin.x = [_texturePlistArray count] * 85;
    texturebutton.frame = texturetemp;
    [_textureScrollView addSubview:texturebutton];
    [_textureScrollView setContentSize:CGSizeMake(texturebutton.frame.origin.x + texturebutton.frame.size.width, 0)];
}
...
-(IBAction)onFilter:(EffectCustomButton *)sender{
    ...
    if (sender.lockedImageView.hidden == YES) {
        _selectTextureIndex = -1;
        ...
        _editSlider.hidden = YES;
        [self alertUnlockFilterWithImageName:[dict objectForKey:@"unlockImage"] andIndex:sender.tag - 1];
    }
}
...
-(IBAction)onTextures:(EffectCustomButton *)sender{
    ...
    if (sender.lockedImageView.hidden == YES) {
        ...
        _editSlider.hidden = YES;
        [self alertUnlockTextureWithImageName:[dict objectForKey:@"unlockImage"] andIndex:sender.tag - 1];
    }
}

-(void)didSuccessBuyProduct:(NSString*)productId{
    [super didSuccessBuyProduct:productId];
    [self onHideUnlockAlert];
    if (_filterScrollView.hidden == NO) {
        for (int i = 0; i< [_filterPlistArray count]; i++) {
            EffectCustomButton *button = [_filterScrollView viewWithTag:i+1];;
            [button setLayoutWithContent:[_filterPlistArray objectAtIndex:i]];
        }
    }else if(_textureScrollView.hidden == NO){
        for (int i = 0; i< [_texturePlistArray count]; i++) {
...
- (void)alertUnlockFilterWithImageName:(NSString *)imageName andIndex:(NSInteger)index{
    ...
    [UIView animateWithDuration: 0.5 delay: 0 usingSpringWithDamping: 0.8 initialSpringVelocity: 0 options: UIViewAnimationOptionCurveLinear animations: ^{
        imageView.center = _unlockBackgroundView.center;
    } completion: nil];
}
...
- (void)onUnlockFilter:(UIGestureRecognizer *)gesture{
    ...
    [self.proManager buyProduct:[[_filterPlistArray objectAtIndex:gesture.view.tag - 1] objectForKey:@"productCode"]];
}

- (void)onUnlockTexture:(UIGestureRecognizer *)gesture{
    [MBProgressHUD showActivityMessageInView:NSLocalizedString(@"Loading", nil)];
    [self.proManager buyProduct:[[_texturePlistArray objectAtIndex:gesture.view.tag - 1] objectForKey:@"productCode"]];
}
...
- (UIButton *)getButtonWithTitle:(NSString *)title andTag:(int)tag{
    ...
    [button setTag:tag];
    //字型为第一个参数 字体大小就是小面的 10
//    UIFont *font = [UIFont fontWithName:@"DS-Digital" size:10];
...
-(IBAction)onSetting:(id)sender{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [_alphaView setUserInteractionEnabled:YES];
...
- (void)onCloseSettingView{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [_alphaView viewWithTag:1].frame = temp;
    } completion:^(BOOL finished) {
